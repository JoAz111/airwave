...
struct StationSearchServiceTests {
    ...
    @Test
    func searchQueriesFourFieldsAndDeduplicatesUUIDs() async throws {
        let directory = DirectoryFake(stations: [Self.station])
        let service = StationSearchService(directory: directory)
        ...
        #expect(Set(await directory.queries.compactMap(\.field)) == Set([.name, .country, .language, .tag]))
    }
